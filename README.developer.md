...
- [Codecov](https://codecov.io/gh/gundestrup/jekyll-client-search) — coverage
  reporting. CI uploads Cobertura XML generated by `simplecov-cobertura` from
  the Ruby 3.4 / Node 24 matrix job; status check thresholds live in
  `codecov.yml`. Uploads authenticate via OIDC (`id-token: write`), with the
  `CODECOV_TOKEN` secret as a fallback.
...
`lines` and `branches` keys per file). In CI (`ENV["CI"]`), SimpleCov emits
Cobertura XML at `coverage/coverage.xml` instead, which is uploaded to
Codecov by the `codecov/codecov-action` step in `ci.yml`.
